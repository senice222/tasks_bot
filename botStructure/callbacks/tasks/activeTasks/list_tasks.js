...
module.exports = function (bot) {
    bot.action(['?list_of_tasks', /\?list_of_tasks_(.+)/], async (ctx) => {
        ...
        pages = pages.map((page, i) => {
            ...
            if (i !== 0) navigationButtons.push({ text: '<<', callback_data: '?list_of_tasks_0' });
            if (i !== 0) navigationButtons.push({ text: '<', callback_data: '?list_of_tasks_' + (i - 1) });
            navigationButtons.push({ text: `[${i + 1}/${pages.length}]`, callback_data: '@' });
            if (i !== pages.length - 1) navigationButtons.push({ text: '>', callback_data: '?list_of_tasks_' + (i + 1) });
            if (i !== pages.length - 1) navigationButtons.push({ text: '>>', callback_data: '?list_of_tasks_' + String(pages.length - 1) });
